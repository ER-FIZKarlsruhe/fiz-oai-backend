...
public class SolrSearchServiceImpl implements SearchService {
    ...
    public SolrSearchServiceImpl() {
        ...
        solrUpdateClient = initSolrUpdateClient();
    }
    ...
    public void updateDocument(Item item) throws IOException {
        ...
        solrUpdateClient.add(solrDocument);
    ...
    public void deleteDocument(Item item) throws IOException {
        solrClient.deleteById(item.getIdentifier());
    }

    @Override
    public SearchResult<String> search(
        Integer rows, String set, String format, Date fromDate, Date untilDate, String resumptionToken) throws IOException {
        
        SolrQuery query = new SolrQuery();
        query.setRequestHandler("/terms");
        query.setTerms(true);
        query.setTermsLimit(2000);
        query.set
        query.set("oauth_consumer_key", "yORA1fcakVD1trPNdeHPAOWLJO5J5LMmZ3d7DiYzB5SZvRyzwCC1380005165202");
        QueryRequest request = new QueryRequest(query);
        List<Term> terms = request.process(initSolrClient()).getTermsResponse().getTerms(field);
        ...
        try {
            ...
            queryBuilder
                .filter(QueryBuilders
                    .rangeQuery("datestamp").from(Configuration.getDateformat().format(finalFromDate))
            ...
            if (reindexAllFuture != null) {
                ...
                while (!reindexAllFuture.isCancelled() && attempt <= stopAttempts) {
                    ...
                    try {
                        ...
                        LOGGER
                            .warn("Attempt " + attempt + " of " + stopAttempts
                                + " to stop the current Reindex process...");
                    }
    ...
    public boolean reindexAll() {
        if (reindexStatus != null && StringUtils.isBlank(reindexStatus.getEndTime())) {
            LOGGER
                .warn("REINDEX status: Reindex process already started since " + reindexStatus.getStartTime()
        ...
        reindexAllFuture = CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                ClusterHealthResponse responseAllIndexes =
                    elasticsearchClient.cluster().health(requestAllIndexes, RequestOptions.DEFAULT);
                ...
                if (StringUtils.isBlank(reindexStatus.getOriginalIndexName())
                    ...
                    LOGGER
                        .error("Not able to determine index names: original (" + reindexStatus.getOriginalIndexName()
                ...
                if (!createIndex(reindexStatus.getNewIndexName(), mapping)) {
                    LOGGER
                        .error("REINDEX status: Something went wrong while creating the new index "
                ...
                do {
                    ...
                    for (final Item pickedItem : bufferListItems) {
                        ...
                        if (mostRecentItem == null) {
                            ...
                            try {
                                if (Configuration
                                    .getDateformat().parse(mostRecentItem.getDatestamp())
                    ...
                    LOGGER
                        .info("REINDEX status: " + reindexStatus.getIndexedCount() + " indexed out of "
                ...
                if (!reindexStatus.isStopSignalReceived()) {
                    ...
                    for (final String pickedIndex : allIndexes) {
                        ...
                        requestDeleteOldAlias
                            .setJsonEntity("{\n" + "    \"actions\" : [\n" + "        { \"remove\" : { \"index\" : \""
                                + pickedIndex + "\", \"alias\" : \"" + ITEMS_ALIAS_INDEX_NAME + "\" } }\n" + "    ]\n"
                                + "}");
                        LOGGER
                            .info("REINDEX status: execute remove alias " + ITEMS_ALIAS_INDEX_NAME + " to "
                                + pickedIndex);
                        try {
                            Response responseDeleteOldAlias = lowLevelClient.performRequest(requestDeleteOldAlias);
                            LOGGER
                                .info("REINDEX status: responseDeleteOldAlias.getStatusLine().getStatusCode()"
                            ...
                            if (e instanceof ResponseException
                                ...
                                LOGGER
                                    .info("REINDEX status: alias " + ITEMS_ALIAS_INDEX_NAME + " to " + pickedIndex
                                        + " not found to delete.");
                            }
                            else {
                                LOGGER
                                    .error("REINDEX status: something went wrong while deleting alias "
                                        + ITEMS_ALIAS_INDEX_NAME + " to " + pickedIndex, e);
                            }
                        }
                    }

                    LOGGER
                        .info("REINDEX status: Add new alias " + ITEMS_ALIAS_INDEX_NAME + " to index "
                            + reindexStatus.getNewIndexName());
                    Request requestNewAlias = new Request("POST", "/_aliases");
                    requestNewAlias
                        .setJsonEntity("{\n" + "    \"actions\" : [\n" + "        { \"add\" : { \"index\" : \""
                            + reindexStatus.getNewIndexName() + "\", \"alias\" : \"" + ITEMS_ALIAS_INDEX_NAME
                            + "\" } }\n" + "    ]\n" + "}");
                    LOGGER.info("REINDEX status: execute new alias");
                    Response responseNewAlias = lowLevelClient.performRequest(requestNewAlias);
                    LOGGER
                        .info("REINDEX status: responseNewAlias.getStatusLine().getStatusCode()"
                ...
                LOGGER
                    .error("REINDEX status: Something went wrong while processing the new index "
                        + reindexStatus.getNewIndexName(), e);
                return false;
            }
            finally {
    ...
    public String getReindexStatusVerbose() {
        ...
        if (reindexStatus == null) {
            ...
            if (startZDT != null) {
                timeLapsed =
                    Duration
                        .between(startZDT, StringUtils.isBlank(reindexStatus.getEndTime())
                            ? ZonedDateTime.now(ZoneOffset.UTC) : ZonedDateTime.parse(reindexStatus.getEndTime()));
            ...
            if (StringUtils.isBlank(reindexStatus.getEndTime()) && percProgress > 0 && totalSecondsSoFar > 0
                ...
                final ZonedDateTime etaZDT =
                    startZDT.plusSeconds((long) estimatedTotalSeconds).withZoneSameInstant(ZoneOffset.UTC);
                if (etaZDT != null) {
                    eta = etaZDT.toString();
                }
            }
    ...
    private ConcurrentUpdateSolrClient initSolrUpdateClient() {
        ConcurrentUpdateSolrClient.Builder builder =
            new ConcurrentUpdateSolrClient.Builder(Configuration.getInstance().getProperty("solr.url"));
        CloseableHttpClient httpclient =
            HttpClients
                .custom().setProxy(null).setMaxConnPerRoute(128).setMaxConnTotal(256)
                .setConnectionTimeToLive(900, TimeUnit.SECONDS).build();
